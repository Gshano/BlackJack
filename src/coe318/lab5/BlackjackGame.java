...
public class BlackjackGame {
  ...
  public int score(CardPile p) {
    ...
    for (Card card : p.getCards()) {
        ...
        if (rank >= 2 && rank <= 10) {
            totalScore += rank; // Number cards are worth their rank
        } else if (rank > 10 && rank < 14) {
            totalScore += 10; // Face cards (Jack, Queen, King) are worth 10
        } else if (rank == 14) { // Ace
            totalScore += 11; // Aces are worth 11, unless adjusted later
            aceCount++;
        }
    }
    ...
    while (totalScore > 21 && aceCount > 0) {
        totalScore -= 10; // Change Ace value from 11 to 1
        aceCount--;
    }
